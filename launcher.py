            ...
            if event.to_me:
                ...
                if request == 'привет':
                    ...
                    if check is None:
                        ...
                        if user_info['city'] == 1:
                            write_msg(event.user_id,
                                      'Я не смог определить твой город, извини. Буду искать, будто ты живешь в '
                                      'Москве. Если ты хочешь изменить свой город - напиши "изменить город"!')
                        if user_info['bdate'] == 0:
                            write_msg(event.user_id,
                                      'Я не смог определить твой год рождения, извини. Буду считать, будто ты родился '
                                      'в 2000 году. Если ты хочешь изменить год рождения - напиши "изменить год"!')
                    ...
                    count = 0
                    clear_match_list = db_application.get_persons_for_show(conn, event.user_id)
                    if not clear_match_list:
                        write_msg(event.user_id, 'У тебя закончились варианты или ты ещё не подбирал кандидатов. '
                                                 'Напиши "искать", чтобы я составил новый список!')
                    else:
                        ...
                        if take_photos is not None:
                            ...
                            write_msg_att(event.user_id,
                                          ...
                                          f'\nНапиши "далее" чтобы посмотреть другие варианты или "добавь" чтобы '
                                          f'добавить человека в избранное',
                                          photos)
                        else:
                            write_msg(event.user_id,
                                      f'Имя: {result["response"][0]["first_name"]}'
                                      f'\nФамилия: {result["response"][0]["last_name"]}'
                                      f'\nСсылка: https://vk.com/id{result["response"][0]["id"]}'
                                      f'\nНапиши "далее" чтобы посмотреть другие варианты или "добавь" чтобы добавить '
                                      f'человека в избранное '
                    ...
                    write_msg(event.user_id, f'Список избранных закончился, чтобы вернуться к просмотру кандидатов напиши "смотреть"!')
